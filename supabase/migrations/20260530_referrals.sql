...
-- Callable by signed-in users missing a code (existing accounts + fallback).
...
-- Backfill referral codes for existing users (bypasses client guard via session flag).
do $$
declare r record;
begin
